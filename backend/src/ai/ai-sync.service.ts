...
export class AiSyncService implements OnModuleInit, OnModuleDestroy {
    ...
    async enqueue(manager: EntityManager, sourceId: string, operation: AiSyncOperation): Promise<AiSyncOutbox> {
    ...
    private async processJob(job: AiSyncOutbox): Promise<void> {
        try {
            ...
            job.attempts += 1;
            job.lastError = error instanceof Error ? error.message.slice(0, 1_000) : 'AI 동기화 실패';
    ...
    private async findPost(sourceId: string): Promise<Post | null> {
        ...
        if (legacyPostId) {
            return this.postRepository.findOne({ where: { id: Number(legacyPostId) } });
        }
