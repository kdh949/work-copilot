...
게시 전에 대상 Confluence space·부모 페이지·제목·본문·근거 인용을 확인하고 해당
단계만 승인합니다. 페이지 생성이 성공해야 Jira remote link와 요약 댓글 미리보기가
열리며, 선택한 하위 작업은 마지막 별도 승인에서만 생성됩니다. 각 단계는 idempotency
key와 외부 객체 ID를 보존해 네트워크 실패 후에도 이미 성공한 작업을 다시 만들지
않습니다.

Confluence 추천은 Jira 이슈 키와 요약을 기준으로 한 metadata 검색만 사용합니다. 외부
문서 본문은 모델 지시로 신뢰하지 않고, 실제 게시 HTML도 서버가 이스케이프해 렌더링합니다.
